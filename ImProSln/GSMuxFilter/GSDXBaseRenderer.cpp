...
// Implements the GSDXBaseRendererer class

GSDXBaseRenderer::GSDXBaseRenderer(REFCLSID RenderClass, // CLSID for this renderer
...
GSDXBaseRenderer::~GSDXBaseRenderer()
{
...
HRESULT GSDXBaseRenderer::GetMediaPositionInterface(REFIID riid, __deref_out void **ppv)
{
...
STDMETHODIMP GSDXBaseRenderer::NonDelegatingQueryInterface(REFIID riid, __deref_out void **ppv)
{
...
HRESULT GSDXBaseRenderer::SourceThreadCanWait(BOOL bCanWait)
{
	if (bCanWait == TRUE) {
...
void GSDXBaseRenderer::DisplayRendererState()
{
...
HRESULT GSDXBaseRenderer::WaitForRenderTime()
{
...
void GSDXBaseRenderer::WaitForReceiveToComplete()
{
	for (;;) {
		if (!m_bInReceive) {
			break;
...
FILTER_STATE GSDXBaseRenderer::GetRealState() {
	return m_State;
}
...
STDMETHODIMP GSDXBaseRenderer::GetState(DWORD dwMSecs,FILTER_STATE *State)
{
...
HRESULT GSDXBaseRenderer::CompleteStateChange(FILTER_STATE OldState)
{
...
STDMETHODIMP GSDXBaseRenderer::Stop()
{
...
STDMETHODIMP GSDXBaseRenderer::Pause()
{
	CAutoLock cRendererLock(&m_InterfaceLock);
...
STDMETHODIMP GSDXBaseRenderer::Run(REFERENCE_TIME StartTime)
{
	CAutoLock cRendererLock(&m_InterfaceLock);
	FILTER_STATE OldState = m_State;
...
int GSDXBaseRenderer::GetPinCount()
{
	if (m_pInputPin == NULL) {
		//  Try to create it
		(void)GetPin(0);
...
CBasePin *GSDXBaseRenderer::GetPin(int n)
{
	CAutoLock cObjectCreationLock(&m_ObjectCreationLock);
	...
	if (m_pInputPin == NULL) {
		...
		m_pInputPin = new GSDXRendererInputPin(this,&hr,L"In");
		if (NULL == m_pInputPin) {
			return NULL;
		}
...
STDMETHODIMP GSDXBaseRenderer::FindPin(LPCWSTR Id, __deref_out IPin **ppPin)
{
...
HRESULT GSDXBaseRenderer::EndOfStream()
{
...
HRESULT GSDXBaseRenderer::BeginFlush()
{
...
HRESULT GSDXBaseRenderer::EndFlush()
{
...
HRESULT GSDXBaseRenderer::CompleteConnect(IPin *pReceivePin)
{
...
HRESULT GSDXBaseRenderer::Active()
{
	return NOERROR;
}
...
HRESULT GSDXBaseRenderer::Inactive()
{
	if (m_pPosition) {
...
HRESULT GSDXBaseRenderer::SetMediaType(const CMediaType *pmt)
{
	return NOERROR;
}
...
HRESULT GSDXBaseRenderer::BreakConnect()
{
...
HRESULT GSDXBaseRenderer::GetSampleTimes(IMediaSample *pMediaSample,
...
HRESULT GSDXBaseRenderer::ShouldDrawSampleNow(IMediaSample *pMediaSample,
...
void GSDXBaseRenderer::SignalTimerFired()
{
	m_dwAdvise = 0;
}
...
HRESULT GSDXBaseRenderer::CancelNotification()
{
...
BOOL GSDXBaseRenderer::ScheduleSample(IMediaSample *pMediaSample)
{
...
HRESULT GSDXBaseRenderer::Render(IMediaSample *pMediaSample)
{
...
BOOL GSDXBaseRenderer::HaveCurrentSample()
{
...
IMediaSample *GSDXBaseRenderer::GetCurrentSample()
{
	CAutoLock cRendererLock(&m_RendererLock);
	if (m_pMediaSample) {
...
HRESULT GSDXBaseRenderer::PrepareReceive(IMediaSample *pMediaSample)
{
...
HRESULT GSDXBaseRenderer::Receive(IMediaSample *pSample)
{
	ASSERT(pSample);
...
HRESULT GSDXBaseRenderer::ClearPendingSample()
{
	CAutoLock cRendererLock(&m_RendererLock);
	if (m_pMediaSample) {
...
{
	GSDXBaseRenderer *pRenderer = (GSDXBaseRenderer *) dwUser;
	NOTE1("EndOfStreamTimer called (%d)",uID);
	pRenderer->TimerCallback();
}

//  Do the timer callback work
void GSDXBaseRenderer::TimerCallback()
{
...
HRESULT GSDXBaseRenderer::SendEndOfStream()
{
...
HRESULT GSDXBaseRenderer::NotifyEndOfStream()
{
	CAutoLock cRendererLock(&m_RendererLock);
...
HRESULT GSDXBaseRenderer::ResetEndOfStream()
{
	ResetEndOfStreamTimer();
	CAutoLock cRendererLock(&m_RendererLock);
...
void GSDXBaseRenderer::ResetEndOfStreamTimer()
{
...
HRESULT GSDXBaseRenderer::StartStreaming()
{
	CAutoLock cRendererLock(&m_RendererLock);
...
HRESULT GSDXBaseRenderer::StopStreaming()
{
	CAutoLock cRendererLock(&m_RendererLock);
	m_bEOSDelivered = FALSE;
...
void GSDXBaseRenderer::SetRepaintStatus(BOOL bRepaint)
{
	CAutoLock cSampleLock(&m_RendererLock);
...
void GSDXBaseRenderer::SendNotifyWindow(IPin *pPin,HWND hwnd)
{
...
void GSDXBaseRenderer::SendRepaint()
{
	CAutoLock cSampleLock(&m_RendererLock);
	ASSERT(m_pInputPin);
...
BOOL GSDXBaseRenderer::OnDisplayChange()
{
...
void GSDXBaseRenderer::OnRenderStart(IMediaSample *pMediaSample)
{
#ifdef PERF
...
void GSDXBaseRenderer::OnRenderEnd(IMediaSample *pMediaSample)
{
#ifdef PERF
...
GSDXRendererInputPin::GSDXRendererInputPin(__inout GSDXBaseRenderer *pRenderer,
									   __inout HRESULT *phr,
...
HRESULT GSDXRendererInputPin::NonDelegatingQueryInterface(REFIID iid, void **ppv)
{
	if (iid == IID_IGSDXSharePin) 
	{
...
STDMETHODIMP GSDXRendererInputPin::EndOfStream()
{
...
STDMETHODIMP GSDXRendererInputPin::BeginFlush()
{
	CAutoLock cRendererLock(&m_pRenderer->m_InterfaceLock);
	{
		CAutoLock cSampleLock(&m_pRenderer->m_RendererLock);
...
STDMETHODIMP GSDXRendererInputPin::EndFlush()
{
	CAutoLock cRendererLock(&m_pRenderer->m_InterfaceLock);
	CAutoLock cSampleLock(&m_pRenderer->m_RendererLock);
...
STDMETHODIMP GSDXRendererInputPin::Receive(IMediaSample *pSample)
{
...
HRESULT GSDXRendererInputPin::BreakConnect()
{
...
HRESULT GSDXRendererInputPin::CompleteConnect(IPin *pReceivePin)
{
	HRESULT hr = S_OK;
	...
	hr = m_pRenderer->CompleteConnect(pReceivePin);
	if (FAILED(hr)) {
		SetConnectedPin(NULL);
		return hr;
	}
...
STDMETHODIMP GSDXRendererInputPin::QueryId(__deref_out LPWSTR *Id)
{
...
HRESULT GSDXRendererInputPin::CheckMediaType(const CMediaType *pmt)
{
...
HRESULT GSDXRendererInputPin::Active()
{
...
HRESULT GSDXRendererInputPin::Inactive()
{
...
HRESULT GSDXRendererInputPin::SetMediaType(const CMediaType *pmt)
{
...
HRESULT GSDXRendererInputPin::GetD3DFilter(IGSDXShareFilter*& pFilter)
{
	pFilter = m_pRenderer;
	return S_OK;
}
