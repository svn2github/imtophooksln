...
float4 mainPS(VSOUT vin) : SV_Target {
	...
	float sGray = gray*gray*g_scaleValue*255.0;
	if (g_nChannel == 1)
	{
		return float4(sGray, sGray, sGray, 1.0);
	}
	float4 colorRet = float4(0,0,0,0);
	colorRet.r = sGray * (colorPt.r*0.3/gray);
	colorRet.g = sGray * (colorPt.g*0.59/gray);
	colorRet.b = sGray * (colorPt.b*0.11/gray);
	colorRet.a = 1.0;
	return colorRet;
}
