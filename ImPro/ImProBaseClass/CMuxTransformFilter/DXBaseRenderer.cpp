...
// Implements the DXBaseRendererer class

DXBaseRenderer::DXBaseRenderer(REFCLSID RenderClass, // CLSID for this renderer
...
HRESULT DXBaseRenderer::GetMediaPositionInterface(REFIID riid, __deref_out void **ppv)
{
...
STDMETHODIMP DXBaseRenderer::NonDelegatingQueryInterface(REFIID riid, __deref_out void **ppv)
{
...
HRESULT DXBaseRenderer::SourceThreadCanWait(BOOL bCanWait)
{
	if (bCanWait == TRUE) {
...
HRESULT DXBaseRenderer::WaitForRenderTime()
{
...
void DXBaseRenderer::WaitForReceiveToComplete()
{
	for (;;) {
		if (!m_bInReceive) {
			break;
...
FILTER_STATE DXBaseRenderer::GetRealState() {
	return m_State;
}
...
STDMETHODIMP DXBaseRenderer::GetState(DWORD dwMSecs,FILTER_STATE *State)
{
...
HRESULT DXBaseRenderer::CompleteStateChange(FILTER_STATE OldState)
{
...
STDMETHODIMP DXBaseRenderer::Stop()
{
...
STDMETHODIMP DXBaseRenderer::Pause()
{
	CAutoLock cRendererLock(&m_InterfaceLock);
...
STDMETHODIMP DXBaseRenderer::Run(REFERENCE_TIME StartTime)
{
	CAutoLock cRendererLock(&m_InterfaceLock);
	FILTER_STATE OldState = m_State;
...
int DXBaseRenderer::GetPinCount()
{
	if (m_pInputPin == NULL) {
		//  Try to create it
		(void)GetPin(0);
...
CBasePin *DXBaseRenderer::GetPin(int n)
{
	CAutoLock cObjectCreationLock(&m_ObjectCreationLock);
	...
	if (m_pInputPin == NULL) {
		...
		m_pInputPin = new DXRendererInputPin(this,&hr,L"In");
		if (NULL == m_pInputPin) {
			return NULL;
		}
...
STDMETHODIMP DXBaseRenderer::FindPin(LPCWSTR Id, __deref_out IPin **ppPin)
{
...
HRESULT DXBaseRenderer::EndOfStream()
{
...
HRESULT DXBaseRenderer::BeginFlush()
{
...
HRESULT DXBaseRenderer::EndFlush()
{
...
HRESULT DXBaseRenderer::CompleteConnect(IPin *pReceivePin)
{
...
HRESULT DXBaseRenderer::Inactive()
{
	if (m_pPosition) {
...
HRESULT DXBaseRenderer::SetMediaType(const CMediaType *pmt)
{
	return NOERROR;
}
...
HRESULT DXBaseRenderer::BreakConnect()
{
...
HRESULT DXBaseRenderer::GetSampleTimes(IMediaSample *pMediaSample,
...
HRESULT DXBaseRenderer::ShouldDrawSampleNow(IMediaSample *pMediaSample,
...
void DXBaseRenderer::SignalTimerFired()
{
	m_dwAdvise = 0;
}
...
HRESULT DXBaseRenderer::CancelNotification()
{
...
BOOL DXBaseRenderer::ScheduleSample(IMediaSample *pMediaSample)
{
...
HRESULT DXBaseRenderer::Render(IMediaSample *pMediaSample)
{
...
BOOL DXBaseRenderer::HaveCurrentSample()
{
...
IMediaSample *DXBaseRenderer::GetCurrentSample()
{
	CAutoLock cRendererLock(&m_RendererLock);
	if (m_pMediaSample) {
...
HRESULT DXBaseRenderer::PrepareReceive(IMediaSample *pMediaSample)
{
...
HRESULT DXBaseRenderer::Receive(IMediaSample *pSample)
{
	ASSERT(pSample);
...
HRESULT DXBaseRenderer::ClearPendingSample()
{
	CAutoLock cRendererLock(&m_RendererLock);
	if (m_pMediaSample) {
...
HRESULT DXBaseRenderer::SendEndOfStream()
{
...
HRESULT DXBaseRenderer::NotifyEndOfStream()
{
	CAutoLock cRendererLock(&m_RendererLock);
...
HRESULT DXBaseRenderer::ResetEndOfStream()
{
	ResetEndOfStreamTimer();
	CAutoLock cRendererLock(&m_RendererLock);
...
void DXBaseRenderer::ResetEndOfStreamTimer()
{
...
HRESULT DXBaseRenderer::StartStreaming()
{
	CAutoLock cRendererLock(&m_RendererLock);
...
HRESULT DXBaseRenderer::StopStreaming()
{
	CAutoLock cRendererLock(&m_RendererLock);
	m_bEOSDelivered = FALSE;
...
void DXBaseRenderer::SetRepaintStatus(BOOL bRepaint)
{
	CAutoLock cSampleLock(&m_RendererLock);
...
void DXBaseRenderer::SendNotifyWindow(IPin *pPin,HWND hwnd)
{
...
void DXBaseRenderer::SendRepaint()
{
	CAutoLock cSampleLock(&m_RendererLock);
	ASSERT(m_pInputPin);
...
BOOL DXBaseRenderer::OnDisplayChange()
{
...
void DXBaseRenderer::OnRenderStart(IMediaSample *pMediaSample)
{
#ifdef PERF
...
void DXBaseRenderer::OnRenderEnd(IMediaSample *pMediaSample)
{
#ifdef PERF
...
DXRendererInputPin::DXRendererInputPin(__inout DXBaseRenderer *pRenderer,
									 __inout HRESULT *phr,
...
HRESULT DXRendererInputPin::NonDelegatingQueryInterface(REFIID iid, void **ppv)
{
	if (iid == IID_IDXBasePin) 
	{
...
STDMETHODIMP DXRendererInputPin::EndOfStream()
{
...
STDMETHODIMP DXRendererInputPin::BeginFlush()
{
	CAutoLock cRendererLock(&m_pRenderer->m_InterfaceLock);
	{
		CAutoLock cSampleLock(&m_pRenderer->m_RendererLock);
...
STDMETHODIMP DXRendererInputPin::EndFlush()
{
	CAutoLock cRendererLock(&m_pRenderer->m_InterfaceLock);
	CAutoLock cSampleLock(&m_pRenderer->m_RendererLock);
...
STDMETHODIMP DXRendererInputPin::Receive(IMediaSample *pSample)
{
...
HRESULT DXRendererInputPin::BreakConnect()
{
...
HRESULT DXRendererInputPin::CompleteConnect(IPin *pReceivePin)
{
	HRESULT hr = S_OK;
...
STDMETHODIMP DXRendererInputPin::QueryId(__deref_out LPWSTR *Id)
{
...
HRESULT DXRendererInputPin::CheckMediaType(const CMediaType *pmt)
{
...
HRESULT DXRendererInputPin::Active()
{
...
HRESULT DXRendererInputPin::Inactive()
{
...
HRESULT DXRendererInputPin::SetMediaType(const CMediaType *pmt)
{
...
HRESULT DXRendererInputPin::GetD3DFilter(IDXBaseFilter*& pFilter)
{
	pFilter = m_pRenderer;
	return S_OK;
}
